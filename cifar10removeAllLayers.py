...
tf.app.flags.DEFINE_string('pm', '333', 'pooling scheme across scales.  Each number specifies the number of scales remaining at each layer. The first number has to be the same as used in --num_scales.')
...
def distorted_inputs():
  ...
  for i in range(2):
    ...
    labelPlace=tf.constant(labelsFromNumpy[i*10000:((i+1)*10000)],name="cifar10label",dtype=tf.float32)
...
def inference(x,out):
  ...
  with tf.variable_scope('fully_connected'):
    print("starting with fully connected")
    # print('\t{} --> {}'.format(out))
    bottom = out
